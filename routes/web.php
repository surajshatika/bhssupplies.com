...
Route::get('/c/{code}', [\App\Http\Controllers\ShortLinkController::class, 'redirect'])
    ->where('code', '[A-Za-z0-9]{4,12}')
    ->name('short_link.redirect');
...
Route::resource('shops', ShopController::class)->only(['create', 'store'])->middleware('handle-demo-login');
...
Route::controller(\App\Http\Controllers\SeoSuiteController::class)->group(function () {
    Route::get('/sitemap.xml',        'publicSitemap');
    Route::get('/robots.txt',         'publicRobots');
    Route::get('/video-sitemap.xml',  'publicVideoSitemap');
    Route::get('/news-sitemap.xml',   'publicNewsSitemap');
    Route::get('/rss.xml',            'publicRss');
    Route::get('/llms.txt',           'publicLlmsTxt');
});
...
Route::get('/customer-products/admin', [IyzicoController::class, 'initPayment'])->name('iyzico.customer_products_admin');
...
Route::get('/hvac-supplies-mississauga', function() {
    return view('frontend.locations.mississauga');
})->name('locations.mississauga');

Route::get('/hvac-supplies-toronto', function() {
    return view('frontend.locations.toronto');
})->name('locations.toronto');

Route::get('/hvac-supplies-brampton', function() {
