        ...
        <div class="col-md-6 text-md-right">
            <div class="btn-group">
                <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="las la-magic"></i> {{ translate('Generate with AI SEO') }}
                </button>
                <div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" href="{{ route('admin.seo_on_page.index') }}" target="_blank">{{ translate('On-Page AI Toolkit') }}</a>
                    <a class="dropdown-item" href="{{ route('admin.seo_off_page.index') }}" target="_blank">{{ translate('Off-Page AI Toolkit') }}</a>
                    <a class="dropdown-item" href="{{ route('admin.seo_optimization.index') }}" target="_blank">{{ translate('Technical SEO Optimization') }}</a>
                </div>
            </div>
        </div>
    </div>
...
<div class="">
    <form class="form form-horizontal mar-top" action="{{route('products.store')}}" method="POST" enctype="multipart/form-data" id="aizSubmitForm">
            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    ...
                    <div class="form-group row" id="category">
                        ...
                        <div class="col-md-8">
                            @php $selectedCategoryIds = old('category_ids', []); @endphp
                            <input type="hidden" name="category_id" id="main_category_id" value="{{ old('category_id', '') }}">
                            <select class="form-control aiz-selectpicker" name="category_ids[]" id="category_ids_select" data-live-search="true" data-selected-text-format="count > 2" multiple required onchange="syncMainCategory(this)">
                                    @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if(in_array($category->id, $selectedCategoryIds)) selected @endif>{{ $category->getTranslation('name') }}</option>
                                    @foreach ($category->childrenCategories as $childCategory)
                                    @include('categories.child_category', ['child_category' => $childCategory])
                                    @endforeach
                                    @endforeach
                                </select>
                                <small class="text-muted">{{ translate('You can select multiple categories. The first selected category will be used as the primary category.') }}</small>
                            </div>
                        </div>
                ...
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        ...
                        <button type="button" id="aiProductDescBtn" class="btn btn-sm btn-outline-primary"
                                onclick="aiAutoFillProductDesc()" title="{{ translate('Auto-fill with AI') }}">
                            <i class="las la-magic mr-1"></i>{{ translate('AI Auto-Fill') }}
                            <span id="aiProductDescSpinner" class="spinner-border spinner-border-sm ml-1 d-none" role="status"></span>
                        </button>
                    </div>
                    <div class="card-body">
                        <div class="form-group row">
                            <label class="col-md-3 col-from-label">
                                ...
                                <button type="button" class="btn btn-xs btn-soft-primary d-block mt-2" onclick="openVisualBuilder('short_description')">
                                    <i class="las la-pen-nib"></i> {{ translate('Visual Builder') }}
                                </button>
                            </label>
                            <div class="col-md-8">
                        ...
                        <div class="form-group row">
                            <label class="col-md-3 col-from-label">
                                ...
                                <button type="button" class="btn btn-xs btn-soft-primary d-block mt-2" onclick="openVisualBuilder('description')">
                                    <i class="las la-pen-nib"></i> {{ translate('Visual Builder') }}
                                </button>
                            </label>
                            <div class="col-md-8">
...
<!--                <div class="card">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="meta_title" id="meta_title" placeholder="{{ translate('Meta Title') }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-3 col-from-label"></label>
                            <div class="col-md-8">
                                <button type="button" class="btn btn-sm btn-soft-primary" onclick="autoGenerateMetaTags()">
                                    <i class="las la-magic"></i> {{ translate('Auto Generate Meta Tags with AI') }}
                                </button>
                                <small class="d-block text-muted mt-1">{{ translate('AI will read the product name and description above to write optimized meta tags.') }}</small>
                            </div>
                        </div>
                        <div class="form-group row">
            ...
            <div class="col-lg-4">
                ...
                @include('backend.seo.components.analyzer')
...
@section('script')
<script>
    function syncMainCategory(select) {
        var first = $(select).val();
        $('#main_category_id').val(first ? first[0] : '');
    }
    $(document).ready(function() {
        syncMainCategory($('#category_ids_select')[0]);
        $('#category_ids_select').on('changed.bs.select', function() {
            syncMainCategory(this);
        });
    });

    window.SITE_BASE_URL          = '{{ rtrim(url("/"), "/") }}';
    window.SEO_META_URL           = '{{ route("admin.seo_optimization.generate_meta") }}';
    window.AI_PRODUCT_CONTENT_URL = '{{ route("admin.seo_optimization.generate_product_content") }}';

    function aiAutoFillProductDesc() {
        var name = $('input[name="name"]').val().trim();
        if (!name) {
            AIZ.plugins.notify('danger', '{{ translate('Please enter a product name first') }}');
            return;
        }
        var btn     = $('#aiProductDescBtn');
        var spinner = $('#aiProductDescSpinner');
        ...
        $.ajax({
            ...
            type: 'POST',
            url: window.AI_PRODUCT_CONTENT_URL,
            data: { name: name },
            success: function(data) {
                if (data.success) {
                    $('textarea[name="short_description"]').summernote('code', data.short_description || '');
                    ...
                    btn.html('<i class="las la-magic mr-1"></i>{{ translate("AI Auto-Fill") }} ' + badge);
                    AIZ.plugins.notify('success', '{{ translate('Descriptions filled successfully!') }}');
                } else {
                    AIZ.plugins.notify('danger', data.message || '{{ translate('AI generation failed') }}');
                }
            },
            error: function() {
...
<script src="{{ static_asset('assets/js/seo-analyzer.js') }}"></script>
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    submitBtn.prop('disabled', false).html('{{ translate('Add Product') }}');
                }
            },
            error: function(xhr) {
                submitBtn.prop('disabled', false).html('{{ translate('Add Product') }}');
                if (xhr.status === 422) {
                    ...
                    $.each(errors, function(field, messages) {
                        var input = $('[name="' + field + '"], [name="' + field + '[]"]').first();
                        input.addClass('is-invalid');
                        input.closest('.form-group').append('<div class="invalid-feedback d-block">' + messages[0] + '</div>');
                    });
                } else {
    ...
    function autoGenerateMetaTags() {
        let productName = $('input[name="name"]').val();
        let productDescription = $('textarea[name="description"]').val();
        // Fallback to Summernote/active editor content if standard textarea is empty
        if (!productDescription) {
            productDescription = $('.aiz-text-editor[name="description"]').val() || $('.note-editable').text();
        }

        if(!productName) {
            AIZ.plugins.notify('warning', window.translations ? window.translations.product_name_required || 'Product name is required for generation.' : 'Product name is required for generation.');
            return;
        }

        let btn = $('.btn-soft-primary[onclick="autoGenerateMetaTags()"]');
        let originalText = btn.html();
        btn.html('<i class="las la-spinner la-spin"></i> Generating...');
        btn.prop('disabled', true);
        ...
        $.ajax({
            ...
            type:"POST",
            url:'{{ route('admin.seo_optimization.generate_meta') }}',
            data:{
               name: productName,
            ...
            success: function(data) {
                if(data.success) {
                    ...
                    AIZ.plugins.notify('success', 'Meta tags automatically generated!');
                } else {
                    AIZ.plugins.notify('danger', data.message || 'Failed to generate meta tags.');
                }
                btn.html(originalText);
                btn.prop('disabled', false);
           },
           error: function() {
                AIZ.plugins.notify('danger', 'An error occurred during generation.');
                btn.html(originalText);
                btn.prop('disabled', false);
           }
       });
