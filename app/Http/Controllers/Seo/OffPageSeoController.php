...
class OffPageSeoController extends Controller
{
    use SeoPayloadTrait;
    ...
    public function index()
    {
        ...
        $features = config('seo.features.off_page', []);
        $runs = $setupRequired ? collect() : SeoRun::where('module', 'off_page')->latest()->limit(15)->get();

        return view('backend.seo.off_page.index', compact('project', 'features', 'runs', 'setupRequired'));
    }
    ...
    public function run(Request $request)
    {
        if (!$this->seoTablesReady()) {
            ...
            return redirect()->route('admin.seo_off_page.index');
        }
        ...
        flash(translate('Off-Page SEO task queued successfully'))->success();
        return redirect()->route('admin.seo_off_page.index');
    }
}
