            ...
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('env_key_update.update') }}" method="POST">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select class="form-control aiz-selectpicker mb-2 mb-md-0" name="MAIL_DRIVER" onchange="checkMailDriver(); this.form.querySelector('[name=MAIL_MAILER]').value = this.value;">
                                <option value="sendmail" @if (env('MAIL_MAILER','smtp') == "sendmail") selected @endif>{{ translate('Sendmail') }}</option>
                                <option value="smtp" @if (env('MAIL_MAILER','smtp') == "smtp") selected @endif>{{ translate('SMTP') }}</option>
                                <option value="mailgun" @if (env('MAIL_MAILER','smtp') == "mailgun") selected @endif>{{ translate('Mailgun') }}</option>
                            </select>
                            <input type="hidden" name="MAIL_MAILER" value="{{ env('MAIL_MAILER','smtp') }}">
                        </div>
                    </div>
                    <div id="smtp">
                        <div class="form-group row">
