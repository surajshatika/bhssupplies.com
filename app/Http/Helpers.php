...
if (!function_exists('optimized_image_url')) {
    function optimized_image_url($url)
    {
        static $resolved = [];

        if (empty($url) || !is_string($url) || preg_match('/^(data:|blob:)/i', $url)) {
            return $url;
        }
        ...
        $path = parse_url($webp_url, PHP_URL_PATH);

        if (!$path) {
            return $resolved[$url] = $url;
        }

        $local_path = public_path(ltrim($path, '/'));

        return $resolved[$url] = file_exists($local_path) ? $webp_url : $url;
    }
}
...
if (!function_exists('get_featured_flash_deal')) {
    ...
    {
        return Cache::remember('featured_flash_deal', 300, function () {
            return FlashDeal::query()->isActiveAndFeatured()
    ...
    function get_system_currency()
    {
        ...
        return $currencies[$code] = Cache::remember('system_currency_' . $code, 86400, function () {
            ...
            if (Session::has('currency_code')) {
                ...
                $query->where('id', get_setting('system_default_currency'));
            }
            return $query->first();
        });
    }
...
if (!function_exists('get_image')) {
    function get_image($image)
    {
        ...
        return optimized_image_url($image_url);
    }
}
...
if (!function_exists('ab')) {
    /**
     * A/B test helper for theme code.
     *
     *   @if(ab('hero_cta') === 'B') ... @else ... @endif
     *
     * Returns the assigned variant key for the current visitor, or the supplied
     * default when the experiment isn't running / found.
     */
    function ab(string $experimentKey, string $default = 'A'): string
    {
        try {
            $svc = app(\App\Services\Marketing\ExperimentService::class);
            return $svc->assignVariant($experimentKey) ?? $default;
        } catch (\Throwable $e) {
            return $default;
        }
    }
