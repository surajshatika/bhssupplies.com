...
class ProductService
{
    ...
    public function update(array $data, Product $product)
    {
        ...
        // Phase 2: auto-sync price + inventory to Amazon if product is actively listed
        $this->dispatchAmazonSyncIfNeeded($product);

        return $product;
    }

    private function dispatchAmazonSyncIfNeeded(Product $product): void
    {
        $amazonProduct = \App\Models\AmazonProduct::where('product_id', $product->id)
            ->where('status', 'active')
            ->first();
        ...
        \App\Jobs\Amazon\SyncAmazonInventoryJob::dispatch($amazonProduct);
        \App\Jobs\Amazon\SyncAmazonPriceJob::dispatch($amazonProduct);
    }
